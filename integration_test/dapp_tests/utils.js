...
const mintCw721 = async (contractAddress, address, id) => {
  ...
  const command = `seid tx wasm execute ${contractAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1uaex --broadcast-mode=block -y --output=json`;
...
const instantiateHubContract = async (
  ...
  const command = `seid tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddress} --from ${adminAddress} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const bond = async (contractAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `seid tx wasm execute ${contractAddress} "${jsonString}" --amount=${amount}uaex --from=${address} --gas=500000 --gas-prices=0.1uaex --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const unbond = async (hubAddress, tokenAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `seid tx wasm execute ${tokenAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1uaex --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const transferTokens = async (tokenAddress, sender, destination, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `seid tx wasm execute ${tokenAddress} "${jsonString}" --from=${sender} --gas=200000 --gas-prices=0.1uaex --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
