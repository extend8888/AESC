...
import (
	...
	customnetwork "github.com/extend8888/aescd/tests/network"
)
...
Example:
	aescd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "aescd", "Home directory of the node's daemon configuration")
...
Example:
	aescd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
// NewTestNetworkFixture returns a new aescd AppConstructor for network simulation tests
