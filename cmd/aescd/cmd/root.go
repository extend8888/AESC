...
import (
	...
	aescdapp "github.com/extend8888/aescd/app"
...
// NewRootCmd creates a new root command for aescd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	// we "pre"-instantiate the application for getting the injected/configured encoding configuration
	// and the CLI options for the modules
	// add keyring to autocli opts
	tempApp := aescdapp.NewAescApp(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
...
func initRootCmd(rootCmd *cobra.Command, evmApp *aescdapp.AESCApp) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	var exampleApp *aescdapp.AESCApp
	...
	if height != -1 {
		exampleApp = aescdapp.NewAescApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = aescdapp.NewAescApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove // TODO:VLAD - Remove appoptions and evmchainid
	}
