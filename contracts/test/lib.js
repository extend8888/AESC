...
async function bankSend(toAddr, fromKey, amount="100000000000", denom="uaex") {
    const result = await execute(`seid tx bank send ${fromKey} ${toAddr} ${amount}${denom} -b block --fees 20000uaex -y`);
    await delay()
    return result
}

async function fundSeiAddress(seiAddr, amount="100000000000", denom="uaex", funder=adminKeyName) {
    return await execute(`seid tx bank send ${funder} ${seiAddr} ${amount}${denom} -b block --fees 20000uaex -y`);
}

async function getSeiBalance(seiAddr, denom="uaex") {
...
async function createTokenFactoryTokenAndMint(name, amount, recipient, from=adminKeyName) {
    const command = `seid tx tokenfactory create-denom ${name} --from ${from} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`
    const output = await execute(command);
    const response = JSON.parse(output)
    const token_denom = getEventAttribute(response, "create_denom", "new_token_denom")
    const mint_command = `seid tx tokenfactory mint ${amount}${token_denom} --from ${from} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`
    await execute(mint_command);

    const send_command = `seid tx bank send ${from} ${recipient} ${amount}${token_denom} --from ${from} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`
...
async function storeWasm(path, from=adminKeyName) {
    const command = `seid tx wasm store ${path} --from ${from} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`
    const output = await execute(command);
    const response = JSON.parse(output)
...
async function instantiateWasm(codeId, adminAddr, label, args = {}, from=adminKeyName) {
    ...
    const command = `seid tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddr} --from ${from} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`;
    const output = await execute(command);
    const response = JSON.parse(output);
    return getEventAttribute(response, "instantiate", "_contract_address");
}

async function proposeCW20toERC20Upgrade(erc20Address, cw20Address, title="erc20-pointer", version=99, description="erc20 pointer",fees="200000uaex", from=adminKeyName) {
    const command = `seid tx evm add-cw-erc20-pointer "${title}" "${description}" ${erc20Address} ${version} 200000000uaex ${cw20Address} --from ${from} --fees ${fees} -y -o json --broadcast-mode=block`
    const output = await execute(command);
    const proposalId = getEventAttribute(JSON.parse(output), "submit_proposal", "proposal_id")
    return await passProposal(proposalId)
}

async function passProposal(proposalId,  desposit="200000000uaex", fees="200000uaex", from=adminKeyName) {
    if(await isDocker()) {
...
async function registerPointerForERC20(erc20Address, fees="20000uaex", from=adminKeyName) {
...
async function registerPointerForERC721(erc721Address, fees="20000uaex", from=adminKeyName) {
...
async function registerPointerForERC1155(erc1155Address, fees="200000uaex", from=adminKeyName) {
...
async function executeWasm(contractAddress, msg, coins = "0uaex") {
    const jsonString = JSON.stringify(msg).replace(/"/g, '\\"'); // Properly escape JSON string
    const command = `seid tx wasm execute ${contractAddress} "${jsonString}" --amount ${coins} --from ${adminKeyName} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`;
    const output = await execute(command);
    return JSON.parse(output);
}

async function associateWasm(contractAddress) {
    const command = `seid tx evm associate-contract-address ${contractAddress} --from ${adminKeyName} --gas=5000000 --fees=1000000uaex -y --broadcast-mode block -o json`;
    const output = await execute(command);
    return JSON.parse(output);
}
