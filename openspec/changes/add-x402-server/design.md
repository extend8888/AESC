...
| 决策项 | 选择 | 说明 |
|--------|------|------|
| 服务名称 | **x402-relayer** | 三合一服务（x402-server + Relayer + Facilitator） |
| 部署模式 | **独立二进制** | 不集成到 seid，通过 EVM RPC 通信 |
| 支付代币 | **USDT** | 从 BSC 跨链到 AESC |
| Facilitator | **本地实现** | 本地 verifier/settler，不依赖外部 Coinbase CDP |
...
│  │  - 验证用户交易 / 广播交易    │                                           │
│  ├──────────────────────────────┤                                           │
│  │  Facilitator (支付层)        │  ← 本地实现，不依赖外部服务                  │
│  │  - 本地 EIP-712 签名验证      │                                           │
│  │  - 调用链上 transferWithAuth │                                           │
│  │  - 启动时校验 DOMAIN_SEPARATOR│                                           │
│  └──────────────────────────────┘                                           │
└─────────────────────────────────────────────────────────────────────────────┘
                                       ▲
                                       │ 跨链桥
...
# Token 合约地址 (必须支持 EIP-3009)
# 向后兼容：也可使用 usdt_precompile
token_contract = "0x0000000000000000000000000000000000001010"

# Token EIP-712 域名称 (用于签名验证，必须与链上合约一致)
token_name = "Tether USD"

# Token EIP-712 域版本 (用于签名验证，必须与链上合约一致)
token_version = "1"
