...
func FixTotalSupply(ctx sdk.Context, k *keeper.Keeper) error {
	...
	weiInUaex, weiRemainder := bankkeeper.SplitUaexWeiAmount(totalWeiBalance)
	if !weiRemainder.IsZero() {
		ctx.Logger().Error("wei total supply has been compromised as well; rounding up and adding to reserve")
		if err := k.BankKeeper().AddWei(ctx, k.AccountKeeper().GetModuleAddress(types.ModuleName), bankkeeper.OneUaexInWei.Sub(weiRemainder)); err != nil {
			return err
		}
		weiInUaex = weiInUaex.Add(sdk.OneInt())
	}
	correctSupply = correctSupply.Add(weiInUaex)
