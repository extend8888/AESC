...
func (k *Keeper) PrepareReplayedAddr(ctx sdk.Context, addr common.Address) {
	...
	if a.Balance != nil && a.Balance.CmpBig(utils.Big0) != 0 {
		uaex, wei := state.SplitUaexWeiAmount(a.Balance.ToBig())
		err = k.BankKeeper().AddCoins(ctx, k.GetSeiAddressOrDefault(ctx, addr), sdk.NewCoins(sdk.NewCoin("uaex", uaex)), true)
		if err != nil {
			panic(err)
		}
