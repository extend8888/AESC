...
func CreateEvmd(chainID string, evmChainID uint64, customBaseAppOptions ...func(*baseapp.BaseApp)) evm.EvmApp {
	defaultNodeHome, err := clienthelpers.GetNodeHomeDirectory(".aescd")
	if err != nil {
		panic(err)
	}
...
// SetupEvmd initializes a new aescd app with default genesis state.
// It is used in IBC integration tests to create a new aescd app instance.
