...
func (k BaseSendKeeper) SubWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if amt.LTE(currentWeiBalance) {
		// no need to change uaex balance
		return k.setWeiBalance(ctx, addr, currentWeiBalance.Sub(amt))
	}
	currentUaexBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	currentAggregatedBalance := currentUaexBalance.Mul(OneUaexInWei).Add(currentWeiBalance)
	...
	uaexBalance, weiBalance := SplitUaexWeiAmount(postAggregatedbalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), uaexBalance), true); err != nil {
		return err
	}
...
func (k BaseSendKeeper) AddWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if postWeiBalance.LT(OneUaexInWei) {
		// no need to change uaex balance
		return k.setWeiBalance(ctx, addr, postWeiBalance)
	}
	currentUaexBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	uaexCredit, weiBalance := SplitUaexWeiAmount(postWeiBalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), currentUaexBalance.Add(uaexCredit)), true); err != nil {
		return err
	}
	return k.setWeiBalance(ctx, addr, weiBalance)
}
