...
sei_tester_res=$(printf "12345678\n" | $seidbin tx wasm store sei_tester.wasm -y --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000uaex --broadcast-mode=block --output=json)
sei_tester_id=$(python3 parser.py code_id $sei_tester_res)

# instantiate
echo "Instantiating..."
tester_in_res=$(printf "12345678\n" | $seidbin tx wasm instantiate $sei_tester_id '{}' -y --no-admin --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000uaex --broadcast-mode=block  --label=dex --output=json)
